import { Address, Cell, Contract, beginCell, contractAddress, ContractProvider, Sender, SendMode, Builder } from "@ton/core";
...
export class Router implements Contract {
  ...
  static payToMessage(owner: Address, tokenAAmount: bigint, walletTokenAAddress: Address, tokenBAmount: bigint, walletTokenBAmount: Address): Cell {
    return beginCell().storeAddress(owner).storeCoins(tokenAAmount).storeAddress(walletTokenAAddress).storeCoins(tokenBAmount).storeAddress(walletTokenBAmount).endCell();
  }
  ...
  static setFeesMessage(jetton0Address: Address, jetton1Address: Address, newLPFee: bigint, newProtocolFee: bigint, newRefFee: bigint, newProtocolFeeAddress: Address): Cell {
    return beginCell()
  ...
  async sendSetFees(
  ...
  ) {
    {
      await provider.internal(via, {
        value,
        sendMode: SendMode.PAY_GAS_SEPARATELY,
        body: Router.setFeesMessage(jetton0Address, jetton1Address, newLPFee, newProtocolFee, newRefFee, newProtocolFeeAddress),
      });
    }
  }
  ...
  async sendCollectFees(provider: ContractProvider, via: Sender, value: bigint, jetton0Address: Address, jetton1Address: Address) {
    {
      await provider.internal(via, {
        value,
        sendMode: SendMode.PAY_GAS_SEPARATELY,
  ...
  static swapMessage(jettonAmount: bigint, fromAddress: Address, walletTokenBAddress: Address, toAddress: Address, expectedOutput: bigint, refAddress?: Address): Cell {
    return beginCell()
      .storeCoins(jettonAmount)
      .storeAddress(fromAddress)
      .storeBit(true)
      .storeRef(beginCell().storeAddress(walletTokenBAddress).storeCoins(expectedOutput).storeAddress(toAddress).storeBit(!!refAddress).endCell())
      .endCell();
  }
  ...
  static swapMessageV2(params: {
    ...
    return beginMessage({ op: 0x7362d09cn }).storeCoins(params.jettonAmount).storeAddress(params.fromAddress).storeBit(true).storeRef(swapPayload.endCell()).endCell();
  }
  ...
  async getPoolAddress(provider: ContractProvider, via: Sender, value: bigint, walletTokenAAddress: Address, walletTokenBAddress: Address) {
    await provider.internal(via, {
      value,
      sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: beginCell().storeAddress(walletTokenAAddress).storeAddress(walletTokenBAddress).endCell(),
    });
  }
}
