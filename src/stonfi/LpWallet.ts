import { Address, Slice, Cell, Contract, ContractProvider, SendMode, Sender, beginCell, contractAddress } from "@ton/core";
...
export function LpWalletConfigToCell(config: LpWalletConfig): Cell {
    return beginCell().storeCoins(config.balance).storeSlice(config.ownerAddress).storeSlice(config.jettonMasterAddress).storeRef(config.jettonWalletCode).endCell();
}
